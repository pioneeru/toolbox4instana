...
ARG CONT_IMG_VER
ENV CONT_IMG_VER=v1.0.38
COPY .bashrc /root
COPY scripts/bin/swat /usr/local/bin/
COPY scripts /opt/toolbox
RUN microdnf install -y git tar wget vi podman container-selinux
...
RUN cd /opt  && \
    ...
    tar xzvf openshift-client-linux-amd64-rhel9.tar.gz -C /usr/local/bin/ && \
